...
**新版更新流程**：網站左側「更新與備份」直接選 Excel／HTML／交換 JSON，預覽逐表差異後套用，保留五版本可回復；新版 HTML 能取回原始 Excel。完整操作、公開站發布與本機一鍵建置見 [更新流程](UPDATE_WORKFLOW.md)。
...
表現統計後續修正見 [統計修正報告](PERFORMANCE_FIX.md)：聯賽跨表採較新快照且去重，缺值改顯示已知小計與涵蓋段數，場均保留相同資料範圍的分母。
比較台亦提供 [世界球員俱樂部全賽事比較](CLUB_COMPARISON.md)，接入 40 名球員的 436 筆正式球季總計；新開比較頁預設全賽事，仍可切換聯賽及巴薩範圍。
