...
TABLE_USES: dict[str, Use] = {
    ...
    "Player_Attr_Snap_O": use(
        ADOPTED, SURFACED, PROVENANCE, columns="非門將屬性／日期／Source_ID／_row",
        output="players[].attrs → 能力值表與六維概覽；comparison.players[].snapshots → 全世界球員日期選擇與逐欄比較"),
    "Player_Attr_Snap_G": use(ADOPTED, SURFACED, PROVENANCE, columns="門將屬性組／日期／Source_ID／_row",
                              output="players[].attrs → 能力值表；comparison.players[].snapshots → 門將快照與逐欄來源"),
